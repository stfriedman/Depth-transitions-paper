...
rawdata <- read_csv(here(basepath, "rate_lat_stats_by_sp_fixed0.5.csv")) %>%
  ...
  mutate(tridepth_small = case_when(
    depth_range_deep > 0 & depth_range_deep < 100 ~ "shallow",
    depth_range_deep >= 100 & depth_range_deep < 900 ~ "intermediate",
    depth_range_deep >= 900 ~ "deep"
  )) %>%
  mutate(tridepth_big = case_when(
    depth_range_deep > 0 & depth_range_deep < 300 ~ "shallow",
    depth_range_deep >= 300 & depth_range_deep < 1100 ~ "intermediate",
    depth_range_deep >= 1100 ~ "deep"
  )) %>%
  rowwise() %>%
