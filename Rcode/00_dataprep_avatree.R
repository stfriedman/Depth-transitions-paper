...
## loading ava tree
avatree <- read.nexus(here("data/Avatree.tre")) 
avatree$tip.label <- str_extract(str_to_title(avatree$tip.label), "[A-Za-z]+_[a-z]+")
dups <- avatree$tip.label[which(duplicated(avatree$tip.label))]
wholetree <- drop.tip(avatree, dups)
